...
async function main() {
  ...
  for (const task of board.tasks) {
    const match = synced.find((s) => s.localId === task.id);
    if (match) {
      task.linearIssue = match.identifier;
      task.linearUrl = match.url;
    }
  }
  ...
  const marker = "## KidNest GitHub upload (KNU) publish";
  setupDoc = setupDoc.includes(marker)
    ? setupDoc.replace(/## KidNest GitHub upload \(KNU\) publish[\s\S]*?(?=\n## |$)/, block.trim())
    : setupDoc + block;
