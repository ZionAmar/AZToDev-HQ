/**
 * Publish or sync KidNest GitHub upload board to Linear (EMET team).
 * Idempotent: renames project, skips issue creation when linearIssue is set.
 *
...
const LEGACY_PROJECT_NAMES = ["KidNest — GitHub Migration"];
...
async function cancelHoldingIssue(teamId) {
  ...
  try {
    ...
    if (state?.type === "canceled") {
      return { identifier: issue.identifier, url: issue.url, canceled: true, already: true };
    }
...
async function main() {
  ...
  const block = [
    ...
    `- **Team:** ${team.name}`,
    `- **Project:** ${board.project}`,
    `- **Issues:** ${synced.length}/4`,
    holdingResult?.identifier
      ? `- **EMET-66:** [${holdingResult.identifier}](${holdingResult.url}) (${holdingResult.already ? "already canceled" : "canceled"})`
      : null,
    "",
    "### Issue URLs",
  ...
  const marker = "## KidNest GitHub upload publish";
  setupDoc = setupDoc.includes(marker)
    ? setupDoc.replace(/## KidNest GitHub upload publish[\s\S]*?(?=\n## |$)/, block.trim())
    : setupDoc + block;
