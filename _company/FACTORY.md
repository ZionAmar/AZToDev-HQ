...
- נדב על ה-PC: מושך מ-GitHub (עם האימות של המחשב) ודוחף inbox/outbox לדלפק — הדלפק לא חירש גם בלי git על ה-VPS.
- שגרות: בוקר + סוף־יום מנועה (שעון ירושלים). Pulse כשהזרימה זזה. מפת ניצחון מול תום: `_company/BEAT_TOM.md`.
