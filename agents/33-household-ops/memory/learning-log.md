...
- do: למשוך מבזקי אמת מתצוגת ה-web של ערוצי טלגרם מבוקשים, לעדכן את הערוץ ב-household.json ולשמור דוח ראיות מסודר.
- dont: לא להשאיר את household.json ריק כשמסופק ערוץ מפורש ולא להסתמך על מידע מומצא.
- note: הופק סיכום מקיף של חדשות הארץ והעולם מערוץ hamivzakk ונשמר באאוטבוקס.
