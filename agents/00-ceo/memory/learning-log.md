...
### 2026-09-09 (founder brief)
- task: Founder shared Gemini's long profile of him and noted Gemini initially forgot aztodev.com — asked to acknowledge and add it to the portfolio overview so development, teaching, and personal drive sit under one house.
- do: Treat a founder-supplied bio as HQ truth, then correct product names against the live site (WorkClock Pro not Clock Work; FiTime; My Tor) and put [aztodev.com](https://aztodev.com) as the umbrella — not a footnote. Write a presentable brief (`_company/FOUNDER_BRIEF.md`) plus an agent-facing `FOUNDER.md`. Answer "what should a reader try first" in the brief itself: site first, then WorkClock Pro as the default product wedge.
- dont: Copy Gemini's product list blindly (LayoutFix ≠ Text Rescuer; Tehilim Baklik and online contracts were missing from Gemini but live on the site). Don't dump family/hardware prefs into marketing copy without an internal vs public split.
- note: Live site check 2026-09-09: telemust.com, workclock.pro, fitime.co.il, mytor.pro, tehilim.aztodev.com, tasks.aztodev.com, Text Rescuer Chrome extension, YouTube @aztodev24.
