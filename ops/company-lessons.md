...
### 2026-09-09 · theater_fixed_in_code_not_lessons
- do: After 4 prose lessons about the same theater symptom in one day, `32-delivery-lead` read the actual code path instead of writing a 5th lesson: `runtime/lib/front-desk.mjs`'s fast/chat lane (`fastNoaReply`, used for short status-shaped Telegram messages) never called `executeDelegateRelay` — so any `DELEGATE:` line it produced was silently discarded, and its LLM had zero enforcement behind the "don't claim contact without DELEGATE" instruction in its own system prompt. Fixed by wiring the fast lane through the same `executeDelegateRelay` + `founderFacingText` choke point the Cloud path already used.
- dont: Treat a system-prompt instruction to an LLM as enforcement. If the founder's exact words ("תפני לקשת") don't reliably trigger `inferRequiredDelegate` because of a Hebrew ל-prefix regex gap, or a claim-stripper regex silently no-ops because of a JS `\b`-after-Hebrew-letter bug, the "fix" was theater too — write a regression test with the founder's literal sentence, not a paraphrase.
- note: Also found `runtime/scripts/test-live-status.mjs` already red on `main` and unrun (no `npm test`/CI wired the six `runtime/scripts/test-*.mjs` files together) — added `runtime/scripts/run-tests.mjs` + `npm test`. `6/6 passed` after the fix. PCI-04/05/06/09/10 board state left untouched — still genuinely open, not marked done without real PIN/Nadav evidence.
